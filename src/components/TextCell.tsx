import React, { Component } from 'react';
import { View, Text, Image, TextInput, KeyboardTypeIOS, KeyboardType } from 'react-native';
import { MarkerValue } from './HavesView'
import { Colors } from './Colors'

interface Props {
    placeholder: string,
    value?: string,
    keyboardType?: KeyboardType | KeyboardTypeIOS,
    markerValue: MarkerValue,
    textChanged: (text: string, markerValue: MarkerValue) => void

}
export class TextCell extends Component<Props, {}> {
    render() {
        var base64Icon = 'data:image/png;base64,';

        return (
            <View style={{ height: 45, flexDirection: 'row', alignItems: 'center' }}>
                <Image source={{ uri: base64Icon }} style={{ marginLeft: 10, marginRight: 15, width: 30, height: 30, resizeMode: 'contain' }} />
                <TextInput placeholder={this.props.placeholder}
                    value={this.props.value}
                    keyboardType={this.props.keyboardType}
                    multiline={false}
                    style={{ alignSelf: 'center', height: 40, width: 175, color: Colors.darkblue }}
                    onChangeText={(text) => this.props.textChanged(text, this.props.markerValue)}
                ></TextInput>
            </View >
        )
    }
}